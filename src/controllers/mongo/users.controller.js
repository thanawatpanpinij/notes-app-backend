...
async function getAllUsers(request, response, next) {
  ...
  if (user.role !== "admin") {
    return response.status(403).json({ success: false, message: "Access denied. No permission." });
  }
...
async function deleteUser(request, response, next) {
  ...
  try {
    ...
    if (!user) {
      return response.status(404).json({ success: false, message: `User with id ${userId} not found` });
    }
    response.json({ success: true, message: `User with id ${userId} deleted successfully.` });
  } catch (error) {
    const customError = new Error("Failed to fetch users");
    customError.status = 500;
    customError.success = false;
